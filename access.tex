...
Prior to assessing the measurement and structural models, the demographic profile of the survey sample was examined. The final dataset comprises $N = 200$ DPIIT-registered early-stage technology startup founders operating across major urban tech hubs in India. Table~\ref{tab:demographics} reports the demographic characteristics of the ventures and their respective founders.

\begin{table}[htbp]
\caption{Sample Demographic Profile (N = 200)}
\label{tab:demographics}
\centering
\begin{tabular}{llcc}
\hline
\textbf{Variable} & \textbf{Category} & \textbf{Frequency} & \textbf{Percentage (\%)} \\ \hline
Venture Age       & < 1 Year & 133 & 66.7 \\
                  & 1--7 Years & 67 & 33.3 \\ \hline
Founder Experience & > 10 Years & 133 & 66.7 \\
                  & $\le$ 10 Years & 67 & 33.3 \\ \hline
Founder Status    & First-time Founder & 100 & 50.0 \\
                  & Repeat Founder & 100 & 50.0 \\ \hline
Sector            & Tech/Software & 100 & 50.0 \\
                  & HealthTech & 67 & 33.3 \\
                  & FinTech/Others & 33 & 16.7 \\ \hline
Founder Age       & 25--34 Years & 50 & 25.0 \\
                  & 35--44 Years & 100 & 50.0 \\
                  & 45--54 Years & 50 & 25.0 \\ \hline
\end{tabular}
\end{table}

The sample reflects a highly early-stage and tech-driven venture cohort, with 66.7\% of the startups operating for less than one year. Despite the early stage of their current ventures, the founding teams exhibit high levels of human capital; 66.7\% of the founders possess more than ten years of industry-specific experience, and exactly half (50.0\%) are repeat entrepreneurs. The sector distribution is led by core software and technology applications (50.0%), followed by HealthTech (33.3%) and FinTech/others (16.7%). In terms of founder age, the cohort is centered around mid-career professionals, with 50.0\% of respondents between 35 and 44 years of age, and equal 25.0\% distributions in the younger (25--34) and older (45--54) brackets.

\subsection{Measurement Model Validation}

We evaluated the reflective measurement model by assessing individual indicator reliability, internal consistency, convergent validity, and discriminant validity. Refined scales were validated using Cronbach's $\alpha$ and Composite Reliability (CR) to establish construct reliability. As summarized in Table~\ref{tab:reliability}, all latent constructs exceeded the standard threshold of 0.70. Cronbach's $\alpha$ values ranged from 0.904 for Competitive Advantage to 0.935 for Startup Survival, and CR values ranged between 0.940 and 0.959, demonstrating strong internal consistency. Convergent validity was established via the Average Variance Extracted (AVE); all construct AVE values were well above the 0.50 benchmark, ranging from 0.839 for Competitive Advantage to 0.885 for Startup Survival, confirming that the indicators successfully capture the variance of their respective latent variables.
...
\begin{tabular}{lccc}
\hline
\textbf{Construct} & \textbf{Cronbach's $\alpha$} & \textbf{CR} & \textbf{AVE} \\ \hline
Innovation Capability (IC)   & 0.934 & 0.958 & 0.883 \\
Competitive Advantage (CA)   & 0.904 & 0.940 & 0.839 \\
Startup Survival (SS)        & 0.935 & 0.959 & 0.885 \\ \hline
\end{tabular}
\end{table}

Discriminant validity was verified using the Fornell-Larcker criterion. This requires that the square root of the AVE for each latent variable exceeds its correlation coefficients with all other variables in the model. As detailed in Table~\ref{tab:fornell}, the bold diagonal values (representing the square roots of the AVEs) systematically exceed the off-diagonal correlation values. This establishes distinct boundaries between the latent constructs, validating that the variables are statistically distinct.
...
\begin{tabular}{lccc}
\hline
\textbf{Construct} & \textbf{IC} & \textbf{CA} & \textbf{SS} \\ \hline
\textbf{Innovation Capability (IC)}   & \textbf{0.940} & & \\
\textbf{Competitive Advantage (CA)}   & 0.724 & \textbf{0.916} & \\
\textbf{Startup Survival (SS)}        & 0.655 & 0.586 & \textbf{0.941} \\ \hline
\end{tabular}
\end{table}

\subsection{Structural Model Analysis and Hypothesis Testing}

Following measurement validation, we evaluated the structural model. Standardized path coefficients ($\beta$), $t$-statistics, and $p$-values were computed via PLS-SEM bootstrapping with 5,000 resamples. The structural model displays high explanatory power, accounting for 55.47\% of the variance in Competitive Advantage ($R^2 = 0.5547$) and 73.46\% of the variance in Startup Survival ($R^2 = 0.7346$), indicating that the model captures a substantial portion of the survival dynamics of Indian tech startups.

Table~\ref{tab:hypotheses} provides a summary of the hypothesis testing results. All six hypothesized paths are statistically significant and empirically supported.

\begin{table}[htbp]
\caption{Structural Path Analysis and Hypothesis Testing Results}
\label{tab:hypotheses}
\centering
\begin{tabular}{clcccc}
\hline
\textbf{Hyp.} & \textbf{Path Relationship} & \textbf{Beta ($\beta$)} & \textbf{$t$-statistic} & \textbf{$p$-value} & \textbf{Result} \\ \hline
H1 & IC $\rightarrow$ CA & 0.7448 & 20.63 & $< 0.001$ & Supported \\
H2 & CA $\rightarrow$ SS & 0.6019 & 14.18 & $< 0.001$ & Supported \\
H3 & IC $\rightarrow$ SS (Direct) & 0.2945 & 6.88 & $< 0.001$ & Supported \\
H4 & IC $\rightarrow$ CA $\rightarrow$ SS (Mediation) & 0.4483 & 11.48 & $< 0.001$ & Supported \\
H5 & FE $\times$ IC $\rightarrow$ SS (Moderation) & 0.2319 & 6.49 & $< 0.001$ & Supported \\
H6 & FS $\times$ IC $\rightarrow$ SS (Moderation) & 0.1452 & 4.15 & $< 0.001$ & Supported \\ \hline
\end{tabular}
\end{table}

For the direct effects, $H_1$ is strongly supported, showing that Innovation Capability significantly and positively influences Competitive Advantage ($\beta = 0.7448$, $t = 20.63$, $p < 0.001$). The substantial effect size ($f^2 = 1.247$) indicates that a startup's internal capacity to deploy product, process, and marketing innovations is a primary engine of market differentiation. $H_2$ is supported, confirming that Competitive Advantage positively influences Startup Survival ($\beta = 0.6019$, $t = 14.18$, $p < 0.001$), with a large effect size ($f^2 = 0.435$). $H_3$ is supported, showing that Innovation Capability directly influences Startup Survival ($\beta = 0.2945$, $t = 6.88$, $p < 0.001$) independent of the mediated pathway, representing a medium effect size ($f^2 = 0.125$).

\subsection{Mediation and Moderation Analysis}

Hypothesis $H_4$ evaluated the mediating role of Competitive Advantage in the relationship between Innovation Capability and Startup Survival. The indirect path coefficient is statistically significant and positive ($\beta = 0.4483$, $t = 11.48$, $p < 0.001$). Since both the direct path (H3) and the indirect path (H4) are significant, Competitive Advantage acts as a \textit{partial mediator}. This suggests that while innovation capabilities directly contribute to startup resilience, a major portion of their survival benefit is realized when they are successfully translated into a defensible competitive market position.

Hypotheses $H_5$ and $H_6$ evaluated the moderating effects of founder demographics and venture funding stage. Both moderation pathways are positive and statistically significant. Specifically, $H_5$ (Founder Experience moderation) is supported ($\beta = 0.2319$, $t = 6.49$, $p < 0.001$), indicating that founder industry experience strengthens the positive impact of innovation capability on venture survival ($f^2 = 0.08$). Experienced founders can leverage industry-specific heuristics, social capital, and risk management capability to deploy innovations more effectively, mitigating the risks of failure. Similarly, $H_6$ (Funding Stage moderation) is supported ($\beta = 0.1452$, $t = 4.15$, $p < 0.001$), demonstrating that external funding relaxes capital constraints and amplifies the survival returns of innovation ($f^2 = 0.08$). Access to capital provides the necessary buffer to absorb innovation failures, scale go-to-market strategies, and secure the operational slack required for long-term viability.
